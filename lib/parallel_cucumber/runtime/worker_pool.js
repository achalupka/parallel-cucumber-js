  ...
  self.start = function(callback) {
          ...
          if (!worker) {
            ...
            worker.on('message', function(message) {
              ...
              if (message.cmd === 'start') {
                ...
                self.emit('report', { workerIndex: message.workerIndex, task: message.task, report: message.report, success: message.success });
              }
    ...
    Async.times(
      ...
      function(workerIndex, next) {
        if (self.options.pauseBetweenThreads) {
            Debug('Pausing for ' + self.options.pauseBetweenThreads + ' seconds');
            Promise(function(resolve) { setTimeout(resolve, self.options.pauseBetweenThreads * 1000)})
            .then(nextTask(workerIndex));
        } else {
